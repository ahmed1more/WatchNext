...
def recommend_similar(movie_id: int, top_n: int = 10, model_dir: Path | None = None) -> list[dict]:
    ...
    similarity_path = source_dir / "content_similarity.pkl"
    movies_path = source_dir / "content_movies.pkl"

    try:
        similarity_df = pd.read_pickle(similarity_path)
        movies = pd.read_pickle(movies_path)
    except Exception:
        train_content_model(source_dir)
        similarity_df = pd.read_pickle(similarity_path)
        movies = pd.read_pickle(movies_path)
