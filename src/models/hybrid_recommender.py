    ...
    def __init__(
        ...
        if similarity_path.exists():
            try:
                ...
                print(f"Warning: failed to load {similarity_path}: {error}. Rebuilding content model...")
                try:
                    content_based.train_content_model(self.model_dir)
                    self.content_similarity = pd.read_pickle(similarity_path)
                except Exception as retry_error:
                    print(f"Warning: content model rebuild failed: {retry_error}")
